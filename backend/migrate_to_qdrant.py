...
# Collection name (keep same as Chroma for consistency)
...
try:
    qdrant.create_collection(
        ...
        vectors_config=VectorParams(size=384, distance=Distance.COSINE),  # Same as Chroma
    )
...
# Get embedding function (same as Chroma uses)
...
for i in range(0, len(chunks), BATCH_SIZE):
    ...
    for idx, chunk in enumerate(batch):
        ...
        embedding = embedding_fn([chunk["text"]])[0]  # Get first (and only) embedding
